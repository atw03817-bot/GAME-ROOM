  ...
  const fetchProduct = async () => {
    try {
      ...
      setFormData({
        ...
        nameEn: product.nameEn || product.name?.en || '',
        descriptionAr: product.descriptionAr || product.description?.ar || '',
      ...
      // Load color images mapping
      if (product.colorImages) {
        setColorImages(product.colorImages)
      }
      
      // Load price additions
      if (product.colorPrices) {
        setColorPrices(product.colorPrices)
      }
      if (product.storagePrices) {
        setStoragePrices(product.storagePrices)
      }
      
      // Load custom options
      if (product.customOptions) {
        setCustomOptions(product.customOptions.map((option, index) => ({
          ...option,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (customOptions.length > 0) {
        productData.customOptions = customOptions.map(option => ({
          name: option.name,
  ...
  return (
    <div>
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 mb-2">✏️ تعديل المنتج</h1>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            ...
            <div className="mb-4 p-4 bg-blue-50 rounded-lg">
              ...
              <p className="text-xs text-blue-600">
                💡 الأسعار أدناه تشمل السعر الأساسي + الإضافات المحددة لكل لون وسعة
              </p>
            </div>
