// إنشاء المدير بالرابط الصحيح
const { MongoClient } = require('mongodb');
const bcrypt = require('bcryptjs');

// الرابط الصحيح الجديد
...
async function createAdmin() {
  ...
  console.log('☁️ قاعدة البيانات: MongoDB Atlas (mobile_store)');
  console.log('');
  ...
  try {
    ...
    // استخدام اسم قاعدة البيانات الصحيح
    const db = client.db('mobile_store'); // mobile_store بدل mobile-store
    ...
    if (existingUser) {
      ...
      const newAdmin = {
        ...
        updatedAt: new Date(),
        lastLogin: null
        // لا نضع email: null لتجنب مشكلة duplicate key
      };
...
// معلومات النظام
console.log('☁️ سكريبت إنشاء حساب المدير - الرابط الصحيح');
